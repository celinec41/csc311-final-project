...
\begin{table}[H]
    ...
    \begin{tabular}{lcc}
        ...
        \midrule
        Part A user KNN ($k=11$)               & 0.6897 & 0.6847 \\
        Part A item KNN ($k=21$)               & 0.6916 & 0.6828 \\
        Best tuned user KNN                     & 0.6897 & 0.6847 \\
        Best tuned item KNN                     & 0.6916 & 0.6828 \\
        50--50 user/item hybrid                 & 0.7041 & 0.6963 \\
        Hybrid + question-difficulty prior only & 0.7049 & 0.7017 \\
        Hybrid + student-ability prior only     & 0.7090 & 0.7003 \\
        \textbf{Final hybrid (both priors, tuned)} & \textbf{0.7115} & \textbf{0.7079} \\
        \bottomrule
    \end{tabular}
...
The final hybrid improves test accuracy by roughly $2.3$ percentage points over the stronger Part A baseline (user KNN, $68.5\% \to 70.8\%$), and by about $1.2$ points over a naive unweighted average of the two Part A models ($69.6\% \to 70.8\%$), confirming that the gain is not simply from averaging two noisy predictors.
...
\textbf{Experiment 1 -- which prior helps?} We compared adding only the question-difficulty prior, only the student-ability prior, and both, to the 50--50 hybrid (Table~\ref{tab:partb_results}). Unlike a naive expectation that only one prior would help, both priors individually improve on the plain hybrid: the question prior alone raises validation accuracy from $70.41\%$ to $70.49\%$ ($69.63\% \to 70.17\%$ test), and the student prior alone raises validation accuracy further to $70.90\%$ ($70.03\%$ test). Combining the two priors in equal proportion ($\beta=0.5$) outperforms either alone, reaching $71.15\%$ validation and $70.79\%$ test accuracy -- so the selected configuration blends both priors rather than relying on a single one. This still matches the underlying intuition that per-student and per-question correctness rates each carry some independent signal beyond what the KNN imputers capture, and mixing the two priors together reduces variance further than either alone.

\textbf{Experiment 2 -- is the gain from the priors, or from a larger search?} We separately tried widening the base KNN grid to include uniform weighting at $k \in \{5,10,\dots,50\}$ for both orientations (40 base configurations instead of 18), on top of the two priors. This raised validation accuracy further but lowered test accuracy, consistent with the validation set (about 1500 rows) being too small to reliably rank a large number of candidate configurations. \emph{(Note: this comparison was run separately from the main hybrid search above and has not yet been re-run against the current code/data snapshot; the exact percentages should be regenerated before the final submission -- see Limitations.)} We therefore kept the base KNN search restricted to the Part A configuration plus distance-weighting, so that the improvement reported in Table~\ref{tab:partb_results} is attributable to the priors rather than to a larger hyperparameter grid.

Figure~\ref{fig:partb_tuning} shows the validation sensitivity of the final hybrid to $\beta$ (question- vs. student-prior weight) and to $(\gamma, \tau)$ (KNN-vs-prior weight and classification threshold) at the selected configuration. The selected $(\gamma,\tau) = (0.7, 0.55)$ again sits at the edge of the searched grid, consistent with our earlier observation that this region does not have a flat, robust optimum. \emph{(Note: the $\beta$-sensitivity panel needs to be regenerated, since the selected $\beta$ changed from $0$ to $0.5$; the current figure file still reflects the old $\beta=0$ selection and should not be trusted until re-plotted.)}
